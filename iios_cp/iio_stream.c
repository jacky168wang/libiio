...
static struct iio_device *tx[4];
static struct iio_device *rx[4];
static struct iio_device *or[4];
static struct iio_channel *rx0_i[4];
static struct iio_channel *rx0_q[4];
static struct iio_channel *rx1_i[4];
static struct iio_channel *rx1_q[4];
static struct iio_channel *or0_i[4];
static struct iio_channel *or0_q[4];
static struct iio_channel *or1_i[4];
static struct iio_channel *or1_q[4];
static struct iio_channel *tx0_i[4];
static struct iio_channel *tx0_q[4];
static struct iio_channel *tx1_i[4];
static struct iio_channel *tx1_q[4];
static struct iio_channel *obschn0[4];
static struct iio_channel *obschn1[4];
static struct iio_buffer  *rxbuf[4];
static struct iio_buffer  *txbuf[4];
...
static void phy_stream_getchip_info()
{
    ...
    if (ifad9371 == 1) {
        LOG_PRINT_L2(mode_verbo, printf("*Phy stream find %d ad9371\n", i));
        ad9xxxchip = i;
        return;
    }
    ...
    LOG_PRINT_L2(mode_verbo, printf("*Phy stream find %d ad9009\n", i));
    ad9xxxchip = i;
}
...
bool phy_stream_cfg_adrv9009_ch(struct iio_device *ptrphy, int chid, enum iodev type)
{
    ...
    if (type == RX) {
        ...
        set_dev_chn_power(ptrphy, 0);  //power down rx1
        set_dev_chn_power(ptrphy, 1);  //power down rx2
...
static void phy_stream_cfg_axi(void)
{
    int i;
    char devicead9009name[][32]= {
        "adrv9009-phy"
    ...
    for (i=0; i<ad9xxxchip; i++) {
        if (ctx != NULL) {
            LOG_PRINT_L2(mode_verbo, printf("* Acquiring stream devices\n"));
            ...
            if (ifad9371 == 1) {
                if (scfg.tx_on) {
                    ...
                    if ((scfg.RFICseq>>i)&0x01) {
                        LOG_PRINT_L1(mode_verbo,printf("chip %d two tx channels are enabled!\r\n", i));
            	...
            	if (scfg.rx_on && scfg.orx_on == 0) {
                    ...
                    if ((scfg.RFICseq>>i)&0x01) {
                        LOG_PRINT_L1(mode_verbo,printf("chip %d two rx channels are enabled!\r\n", i));
                    ...
                    if (obschn0[i] != NULL) {
                        ...
                        ASSERT(get_stream_chn(or[i], RX, 0, 'i', &or0_i[i]) && "ORX chan i not found");
                        ASSERT(get_stream_chn(or[i], RX, 0, 'q', &or0_q[i]) && "ORX chan q not found");
                        if ((scfg.RFICseq>>i)&0x01) {
                            iio_channel_enable(or0_i[i]);
                            iio_channel_enable(or0_q[i]);
                        }
                    }
                }
            }
            else
            {
                ...
                if (scfg.orx_on) {
                    ...
                    if (obschn1[i] != NULL) {
                        ...
                        ASSERT(get_stream_chn(or[i], RX, 1, 'i', &or1_i[i]) && "ORX chan i not found");
                        ASSERT(get_stream_chn(or[i], RX, 1, 'q', &or1_q[i]) && "ORX chan q not found");
                        if ((scfg.RFICseq>>i)&0x01) {
                            iio_channel_enable(or1_i[i]);
                            iio_channel_enable(or1_q[i]);
                        }
                    }
                }
...
static FILE* phy_stream_analyze_rxfile(char *rxfilename)
{
    ...
    LOG_PRINT_L1(mode_verbo, printf("rxfilename is %s\n", rxfilename));

    if (scfg.rx_on)
    {
    	if (!(wfd = fopen(rxfilename, "wb+")))
       {
...
static FILE* phy_stream_analyze_txfile(char *txfilename)
{
    ...
    LOG_PRINT_L1(mode_verbo, printf("txfilename is %s\n", txfilename));
    if (scfg.tx_on) {
        if (!(rfd = fopen(txfilename, "rb"))) {
            fprintf(stderr, "Cannot open input txfile\n");
            return rfd;
        }
        fseek(rfd, 0, SEEK_END);
        scfg.frame_size = ftell(rfd) / 4; //FIXME: preset sample size = 4; a nasty trick is needed to get real-time sample size
...
static void phy_stream_push_iio_tx_buffer(FILE* rfd)
{
    int i, j;
    char *p_dat, *p_end;
    char *temp = NULL;
    ...
    if(i < ad9xxxchip && txbuf[i])
    {
        ...
        LOG_PRINT_L2(mode_verbo, printf("AdiChip %d begin: %x, end: %x, step: %x\n", i, (unsigned int)p_dat, (unsigned int)p_end, (int)p_inc));

        fread(temp, 4, scfg.frame_size, rfd);
        LOG_PRINT_L1(mode_verbo, printf("AdiChip %d Read %d bytes from %d\n", i, (int)scfg.frame_size, (int)rfd));
        ...
        if (nbytes_tx < 0)  {
            ...
            LOG_PRINT_L1(mode_verbo, printf("AdiChip %d push %d bytes of data\n", i, (int)nbytes_tx));
        }
    }
    if (temp)
        free(temp);
...
static void phy_stream_refill_iio_rx_buffer(FILE* wfd)
{
    int i;
    char *p_dat, *p_end;
    ptrdiff_t p_inc;
    size_t count = 0, bytes_count[4] = {0,0,0,0}, nbytes_rx = 0;
    ...
    while(count < scfg.frame_num) {
        ...
        if (i < ad9xxxchip && rxbuf[i]) {
            ...
            fwrite(p_dat, p_inc, scfg.frame_size, wfd);
            fflush(wfd);
        }
        count++;            
    }
    ...
    if (ifad9371 == 1 && (strlen(rxfilename) || strlen(txfilename))) {
        ...
        if (scfg.enb_DMA) {
            if (scfg.tx_on && strlen(txfilename)) {
                rfd = phy_stream_analyze_txfile(txfilename);
                if (rfd != NULL) {
                    phy_stream_create_libiio_buffer(TX);

                    phy_stream_push_iio_tx_buffer(rfd);
            ...
            if (scfg.rx_on) {
                ...
                wfd = phy_stream_analyze_rxfile(rxfilename);
                if (wfd != NULL) {
                    ...
                    phy_stream_refill_iio_rx_buffer(wfd);
                }
            }
        }
        pause();
        ...
        if (scfg.cycle == 1) pause();
    }

    if (wfd != NULL) { fclose (wfd); wfd = NULL; }
    if (rfd != NULL) { fclose(rfd); rfd = NULL; }
    if(ifad9371 == 1)
