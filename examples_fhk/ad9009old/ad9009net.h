...
#ifndef TALISE_ADRV9009_NETWORK_H_
#define TALISE_ADRV9009_NETWORK_H_
...
/* cleanup and exit */
void shutdown_adrv9009(int flag)
{
    //if (mapped_dev_base){
...
bool cfg_adrv9009_streaming_ch(struct iio_context *ctx, struct channel_cfg *cfg,  int chid, int tx1rx0)
{
    ...
    if(tx1rx0)	//tx1rx0=1: config Tx
    {
        ...
        //ASSERT_ATTR(iio_channel_attr_write_bool(chn, "pa_protection_en", cfg->pa_protection));


        iio_channel_attr_read_double(chn, "hardwaregain", &atten);
        printf("Channel %s Tx attenuation is %f\n", iio_channel_get_name(chn), atten);
        //iio_channel_attr_read_bool(chn, "pa_protection_en", &pap_en);
        //printf("Channel %s PA protection is %d\n", iio_channel_get_name(chn), pap_en);

        // Configure TRx & ORX LO channel
        //printf("* Acquiring ADRV9009 ORx-LO channel\n");
        //chn = iio_device_find_channel(get_adrv9009_phy(ctx), get_ch_name("altvoltage", 0), true);
        //ASSERT_ATTR(iio_channel_attr_write_longlong(chn, "AUX_OBS_RX_LO_frequency", cfg->lo_freq));
        //iio_channel_attr_read_longlong(chn, "AUX_OBS_RX_LO_frequency", &val);
    ...
    else    //tx1rx0=0: config Rx
    {
        // Configure phy RX or ORX
        ...
        //only RX1&RX2 have attr: hd2_tracking_en
        if(chid<2){ //chid=0: Rx1  chid=1: Rx2
            //ASSERT_ATTR(iio_channel_attr_write_bool(chn, "hd2_tracking_en", cfg->rx_hd2tracking));
        }
