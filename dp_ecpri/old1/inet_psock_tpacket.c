...
static void walk_v1_v2_tx(int sock, struct ring *ring)
{
	...
	ll.sll_ifindex = if_nametoindex(g_ifname);
...
static void __v1_v2_fill(struct ring *ring, unsigned int blocks)
{
...
#else
	ring->req.tp_frame_size = TPACKET_ALIGNMENT << 8; // 4K
	bug_on(ring->req.tp_frame_size < DATA_LEN);
#endif
	...
	printf("%s: tp_block_size=%u, tp_frame_size=%u,"\
		"tp_block_nr=%u, req.tp_frame_nr=%u\n", __func__, 
...
static void bind_ring(int sock, struct ring *ring)
{
	...
	ring->ll.sll_ifindex = if_nametoindex(g_ifname);
...
int main(int argc, char *argv[])
{
	...
	printf("#### TX %u in %lu us; throughput %7.3f Mbps, avg.lentency %7.3f us\r\n",
		NUM_PACKETS, tm_us, NUM_PACKETS/tm_us*142*8, tm_us/NUM_PACKETS);
