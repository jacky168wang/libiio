...
#define ORX_ATTEN 0x08
#define CTRL_MEM 0xff200000
...
int OpenSocket(int port)
{
    ...
    memset((char *) &addr_host, 0, sizeof(addr_host));
    ...
    if( bind(s , (struct sockaddr*)&addr_host, sizeof(addr_host) ) == -1)
    {
        die("bind");
    }else{
        printf("Succeed to open and bind socket\n");
        return s;
    }
}
...
int main (int argc, char **argv)
{
    ...
    memset((char*) &addr_client, 0 ,slen);
    ...
    while(1){
        if ((recv_len = recvfrom(s, cmd, 256, 0, (struct sockaddr *) &addr_client, &slen)) == -1)
        {
            die("recvfrom()");
        }else
            printf("Command is %s\n. Come from address %s\n", cmd, inet_ntoa(addr_client.sin_addr));

        if (!strcmp(cmd,"End_Transmission")){
            shutdown_adrv9009(0);
            continue;
        }

        if (!strcmp(cmd,"End_Test")){
            shutdown_adrv9009(1);
            break;
        }
        ...
        command_parse(narg, args, &cfg, &scfg);
        ...
        //memmap = OpenMemory(CTRL_MEM, MEM_MAP_SIZE);
        ...
        //Configuring ADRV9009 Tx attr
        if(scfg.tx_on){
            printf("* Configuring ADRV9009 for Tx streaming\n");
        ...
        //Configuring ADRV9009 Rx&ORX attr
        if (scfg.rx_on){
            ...
            ASSERT(cfg_adrv9009_streaming_ch(ctx, &cfg, 2, 0) && "ORX port 0 not found"); //ORX1
            ASSERT(cfg_adrv9009_streaming_ch(ctx, &cfg, 3, 0) && "ORX port 1 not found"); //ORX2
        }
        ...
        if(scfg.rx_on){
            ...
            if(scfg.rx1==0){//rx1
                printf("* Initializing ADRV9009 IIO Rx streaming channels\n");
        ...
        if (scfg.tx_on){
            ...
            if (nbytes_tx < 0) { printf("Error pushing buf %d\n", (int) nbytes_tx); shutdown_adrv9009(1); }
        ...
        if (scfg.rx_on){
            ...
            while (count < scfg.frame_num){
                ...
                if (nbytes_rx < 0) { printf("Error refilling buf %d\n",(int) nbytes_rx); shutdown_adrv9009(1);}
