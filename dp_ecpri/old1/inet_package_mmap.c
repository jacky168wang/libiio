...
int main( int argc, char ** argv )
{
	...
	/* bind port */
	ec = bind(fd_socket, (struct sockaddr *)&my_addr, sizeof(struct sockaddr_ll));
	if (ec == -1) {
		perror("bind");
		return EXIT_FAILURE;
	}
	...
	ec = setsockopt(fd_socket, SOL_PACKET, PACKET_TX_RING,
	...
	if (c_sndbuf_sz) {
		...
		ec = setsockopt(fd_socket, SOL_SOCKET, SO_SNDBUF,
	...
	ps_header_start = mmap(0, size, PROT_READ|PROT_WRITE,
	...
	ec = pthread_create(&t_fill, &t_attr_fill, task_fill, (void *)ps_header_start);
	if (ec != 0) {
		perror("pthread_create()");
		abort();
	}
...
void *task_send(void *arg)
{
	...
	do {
		...
		ec_send = sendto(fd_socket, NULL, 0, blocking? 0 : MSG_DONTWAIT,
...
void *task_fill(void *arg)
{
	...
	for (ipkt=1; ipkt <= c_packet_nb; ipkt++) {
		...
		/* get free buffer */
		do {
			ps_header = (struct tpacket_hdr *)((void *)ps_header_start + c_buffer_sz*ibuf);
			data = (void*) ps_header + data_offset;
			switch((volatile uint32_t)ps_header->tp_status) {
			case TP_STATUS_AVAILABLE:
				/* fill data in buffer */
				if(first_loop) {
					for(j=0;j<c_packet_sz;j++) data[j] = j;
				}
				loop = 0;
				break;
