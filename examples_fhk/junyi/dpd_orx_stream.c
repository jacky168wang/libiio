...
int main (int argc, char **argv)
{
	...
	//printf("ad9009 TX channel 1 attrs\n");
    //print_adrv9009_channel_attr(ctx, TX, 0);
	//printf("ad9009 TX channel 2 attrs\n");
    //print_adrv9009_channel_attr(ctx, TX, 1);
	//printf("ad9009 RX channel 1 attrs\n");
    //print_adrv9009_channel_attr(ctx, RX, 0);
	//printf("ad9009 RX channel 2 attrs\n");
    //print_adrv9009_channel_attr(ctx, RX, 1);
	//printf("ad9009 ORX channel 1 attrs\n");
    //print_adrv9009_channel_attr(ctx, RX, 2);
	//printf("ad9009 ORX channel 2 attrs\n");
    //print_adrv9009_channel_attr(ctx, RX, 3);
    ...
    size_t rx_samp_num = 122880*20;//5*30720;//10*1024;
    if(scfg.rx_on){
        ...
        if(scfg.rx1 < 2){ //one device only one buffer
            ...
            rxbuf = iio_device_create_buffer(rx, rx_samp_num, false);   //rx_samp_num:The number of samples that the buffer should contain
        }
        else{
    ...
    if (scfg.rx1>1){//orx1 or orx2
        printf("The DPD ORx channel (ORx2) has enabled\n");
        if (dst_fd)
            fclose(dst_fd);
    }
