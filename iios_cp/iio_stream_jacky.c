...
static void usage(void)
{
	fprintf( stderr,
		"Usage: ./a.out [OPTION]\n"
		" -h\tshow this help\n"		
		" -p\tmask of the ports to be enabled\n"
		" -m\tset samples number[4~1048576] for iio RX each time\n"
		" -n\tset samples number[4~1048576] for iio TX each time\n"
		" -x\tenable TX datapath only\n"
...
static void get_args(int argc, char **argv)
{
	int c;
	opterr = 0;
	while ((c = getopt(argc, argv, "p:m:n:x:w:b:v:h")) != EOF) {
		switch (c) {
...
static struct iio_device *get_phy_dev(struct iio_context *ctx, int idev)
{
	...
	dev = iio_context_find_device(ctx, devname[d][16]);
	ASSERT((NULL != dev) && "the specified iio device not found");
	return dev;
}

/* finds streaming IIO channels */
static struct iio_channel *get_iio_chn_ss(struct iio_device *dev, enum iodev d, int ich, char modify)
{
...
static struct iio_channel *get_phy_chn_rf(struct iio_device *dev, enum iodev d, int ich)
{
	struct iio_channel *chn;
	dev = get_phy_dev(ctx, PHY);
	chn = iio_device_find_channel(dev, chnname("voltage", ich), d == TX);
	return chn;
}

/* finds AD9371 local oscillator IIO configuration channels */
static struct iio_channel *get_phy_chn_lo(struct iio_device *dev, enum iodev d, int ich)
{
	struct iio_channel *chn;
	dev = get_phy_dev(ctx, PHY);
	switch (d) {
	...
	case RX:
		chn = iio_device_find_channel(dev, chnname("altvoltage", 0), true);
	case TX:
		chn = iio_device_find_channel(dev, chnname("altvoltage", 1), true);
	default:
		ASSERT(0);
		return NULL;
	}
...
static bool set_phy_chn_ss(enum iodev d, struct iio_channel *chn)
{
	bool ret;
...
static bool set_phy_chn_lo(enum iodev d, struct iio_channel *chn)
{
	struct stream_cfg *cfg;
...
static bool cfg_phy_chn_stream(struct iio_context *ctx, struct stream_cfg *cfg, enum iodev type, int ich)
{
	...
	if (!get_iio_chn_ss(ctx, type, ich, &chn)) {
		return false;
	}
    set_phy_chn_ss(ctx, type, chn);

	// Configure LO channel
	if (mode_verbo > 0)
		printf("* Acquiring AD9371 phy %s lo channel\n", type ? "TX" : "RX");
	if (!get_phy_chn_lo(ctx, type, &chn)) {
		return false;
	}
	set_phy_chn_lo(ctx, type, chn);
...
static bool dump_ad9371_phy(int igrp, enum iodev type, int iprt)
{
	...
	if (mode_verbo > 0)
	    printf("* Acquiring AD9371 phy %s voltage channel %d\n", type ? "TX" : "RX", iprt);
    if (!get_iio_chn_ss(ctx, type, iprt, &chn)) { return false; }
	...
	if (mode_verbo > 0)
	    printf("* Acquiring AD9371 phy %s LO channel\n", type ? "TX" : "RX");
    if (!get_phy_chn_lo(ctx, type, &chn)) { return false; }
...
int init_ad9371_rxpath(int igrp, size_t smp_nb)
{
	...
	rxdev[igrp] = iio_context_find_device(ctx, devname[RX][16]);
	ASSERT((NULL != rxdev[igrp]) && "No rxdev found");
	...
	for (iprt=0; iprt<DEV_PORT_NB; iprt++) {
		if (ports_mask & (1 << iprt)) {
			...
			iiopr_chni[igrp*DEV_PORT_NB+iprt] = get_iio_chn_ss(rxdev[igrp], RX, iprt, 'i');
			ASSERT(ret && "No rxdev_p0i found");
			iiopr_chnq[igrp*DEV_PORT_NB+iprt] = get_iio_chn_ss(rxdev[igrp], RX, iprt, 'q');
...
int init_ad9371_txpath(int igrp, size_t smp_nb)
{
	...
	txdev[igrp] = iio_context_find_device(ctx, devname[TX][16]);
	ASSERT((NULL != txdev[igrp]) && "No txdev found");
		...
		if (ports_mask & (1 << iprt)) {
            if (mode_verbo > 0)
                printf("AD9371_TX(group%d): Initilizing channels of TX-JESD port%d\n",
                    igrp, iprt);
			ret = get_iio_chn_ss(ctx, TX, txdev[igrp], 2*iprt,   0, 
				&iiopt_chni[igrp*DEV_PORT_NB+iprt]);
			ASSERT(ret && "No txdev_p0i found");
			ret = get_iio_chn_ss(ctx, TX, txdev[igrp], 2*iprt+1, 0, 
				&iiopt_chnq[igrp*DEV_PORT_NB+iprt]);
...
void libiio_app_startup(int rxdev_smp_nb, int txdev_smp_nb)
{
	...
	for (igrp=0; igrp<TOT_CHIP_NB; igrp++) {
        ...
        phy = get_phy_dev(ctx, "adrv9371-phy");
		ASSERT((ret > 0) && "No devices");
        if (mode_verbo > 0)
